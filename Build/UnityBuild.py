...
log_file = 'D:/HelloWorld/HelloWorld/build.log'
apk_path = 'D:\HelloWorld\Build\HelloWorld.apk'
...
def call_unity_static_func(func):
    ...
    if os.path.exists(log_file):
        os.remove(log_file)
    time.sleep(1)
    cmd = 'start %s -quit -batchmode -projectPath %s -logFile %s -executeMethod %s --path:%s --version:%s'%(unity_exe,project_path,log_file,func,apk_path,sys.argv[1])
    os.system(cmd)
