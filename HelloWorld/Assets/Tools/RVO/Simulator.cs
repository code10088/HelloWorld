    ...
    public class Simulator
    {
        ...
        private class Worker
        {
            ...
            internal void step(object _)
            {
                ...
                {
                    if (agents[agentNo].active)
                    {
                        agents[agentNo].computeNeighbors();
                        agents[agentNo].computeNewVelocity();
                    }
                    else
                    {
                        agents.RemoveAt(agentNo);
                        agentNo--;
                    }
                }
        ...
        internal List<Agent> waitAdd;
        internal List<int> waitRemove;
        ...
        {
            if (agents_.TryGetValue(agentId, out Agent target))
            {
                target.active = false;
                waitRemove.Add(agentId);
            }
        }
        ...
        public void Clear()
        {
            ...
            waitAdd = new List<Agent>();
            waitRemove = new List<int>();
            defaultAgent_ = null;
            kdTree_ = new KdTree();
        ...
        public float doStep()
        {
            ...
            {
                int index = workerIndex++ % waitAdd.Count;
        ...
        public void SetNumWorkers(int numWorkers)
        {
            ...
            doneEvents_ = new ManualResetEvent[numWorkers_];

            for (int block = 0; block < numWorkers_; ++block)
            {
