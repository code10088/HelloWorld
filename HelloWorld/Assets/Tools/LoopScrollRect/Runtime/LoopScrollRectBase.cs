...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected List<CustomLoopItem> items = new List<CustomLoopItem>();
        ...
        public int GetLastItem(out float offset)
        {
            ...
            if (itemTypeEnd > itemTypeStart)
            {
                int totalChildCount = items.Count;
                float size = GetSize(items[totalChildCount - idx - 1].rt, false);
                while (size + offset <= 0 && itemTypeStart < itemTypeEnd - idx - contentConstraintCount)
                {
                    offset += size;
                    idx += contentConstraintCount;
                    size = GetSize(items[totalChildCount - idx - 1].rt);
                }
            }
            offset = -offset;
        ...
        protected abstract void ProvideData(CustomLoopItem item, int index);
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    {
                        ProvideData(items[i], itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int endItem = 0, bool alignStart = false)
        {
            ...
            ReturnToTempPool(!reverseDirection, items.Count);
        ...
        protected float NewItemAtStart(bool includeSpacing = true)
        {
            ...
            {
                itemTypeStart--;
                CustomLoopItem newItem = GetFromTempPool(itemTypeStart);
                SetSiblingIndex(newItem, deletedItemTypeStart);
                size = Mathf.Max(GetSize(newItem.rt, includeSpacing), size);
            }
        ...
        protected float DeleteItemAtStart()
        {
            ...
            int availableChilds = items.Count - deletedItemTypeStart - deletedItemTypeEnd;
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                RectTransform oldItem = items[deletedItemTypeStart].rt;
        ...
        protected float NewItemAtEnd(bool includeSpacing = true)
        {
            ...
            {
                CustomLoopItem newItem = GetFromTempPool(itemTypeEnd);
                SetSiblingIndex(newItem, items.Count - deletedItemTypeEnd - 1);
                size = Mathf.Max(GetSize(newItem.rt, includeSpacing), size);
                itemTypeEnd++;
                if (totalCount >= 0 && itemTypeEnd >= totalCount)
                {
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                RectTransform oldItem = items[m_Content.childCount - deletedItemTypeEnd - 1].rt;
                size = Mathf.Max(GetSize(oldItem), size);
        ...
        protected void SetSiblingIndex(CustomLoopItem item, int insert)
        {
            if (insert < 0)
            {
                items.Add(item);
                item.SetSiblingIndex(insert);
            }
            else
            {
        ...
        protected void SetSiblingIndex(int remove, int insert)
        {
            if (remove < items.Count)
            {
                var temp = items[remove];
                items.RemoveAt(remove);
                items.Insert(insert, temp);
                temp.SetSiblingIndex(insert);
            }
        }

        protected int deletedItemTypeStart = 0;
        protected int deletedItemTypeEnd = 0;
        protected abstract CustomLoopItem GetFromTempPool(int itemIdx);
